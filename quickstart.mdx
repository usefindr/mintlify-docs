...
- An API key from [Cortex](https://useCortex.com/) (`Authorization: Bearer <your_api_key>`)
...
```mdx
curl -X POST 'https://api.useCortex.com/upload/scrape_webpage?web_url=https://example.com&tenant_id=your_tenant_id' \
...
```mdx
curl -X POST 'https://api.useCortex.com/search/qna' \
  -H "Authorization: Bearer <your_api_key>" \
