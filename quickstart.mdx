...
You can use the `metadata` parameter to restrict the context to only sources matching a specific title or type. For example, `{ "source_title": "contract.pdf" }` will only use sources with that title for answering the question.
