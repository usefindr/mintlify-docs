...
<Tabs>
  <Tab title="API Request">
    ```bash
    curl -X 'POST' \
  'https://api.usecortex.ai/ingestion/upload-document' \
  -H 'accept: application/json' \
  -H 'Content-Type: multipart/form-data' \
  -F 'tenant_metadata=string' \
  -F 'document_metadata_list=string' \
  -F 'upsert=true' \
  -F 'tenant_id=string' \
  -F 'sub_tenant_id=' \
  -F 'document_metadata=string' \
  -F 'relations=false' \
  -F 'files=@temp-test.pdf;type=application/pdf' \
  -F 'tenant_metadata_list=string' \
  -F 'file_id=string'
      ```
  </Tab>
