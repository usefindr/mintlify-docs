...
Delete a specific sub-tenant from a given tenant. This endpoint validates that the sub-tenant is not the primary/default sub-tenant before deletion.
...
- **Primary Tenant**: You cannot delete the main tenant itself using this endpoint
...
### Cannot Delete Default Sub-Tenant
```json
{
  "success": false,
  "message": "Cannot delete the default sub-tenant",
  "data": null,
  "error_code": "CANNOT_DELETE_DEFAULT_SUB_TENANT"
}
```
...
**400 Bad Request**: 
- Attempting to delete the default sub-tenant
- Invalid tenant_id or sub_tenant_id format
...
- Sub-tenant does not exist within the specified tenant
