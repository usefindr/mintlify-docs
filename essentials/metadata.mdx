...
1. **`tenant_metadata`**: Organization-level metadata that applies across all documents within a tenant
{/* 2. **`document_metadata`**: Document-specific metadata that describes individual documents */}

This separation enables powerful multi-tenant scenarios while maintaining document-level granularity.
