---
title: "Building an AI Chief of Staff with Cortex - n8n with NLP (In Progress 🚧)"
...
```
┌───────────────┐   ask(task)   ┌─────────────────────┐   call(fn)   ┌──────────────────┐
│  User / Agent │──────────────▶│  Action Orchestrator│─────────────▶│ Workspace Apps   │
└───────────────┘               │  • Policy Engine    │              │ (Slack, Jira …)  │
        ▲                       │  • Retry / Logging  │              └──────────────────┘
        │                       │  • Auth Vault       │
  feedback / events             │  • Function Cache   │
        │                       └─────────┬───────────┘
        │                                 │
...
Use the *same* `/upload/upload_app_sources` endpoint you already know—set `type: "function"`.
...
> **Notable Flags**
> - `auto_agent_routing`: Lets Cortex choose between *answering* vs *acting*.
